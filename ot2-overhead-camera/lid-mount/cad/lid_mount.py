...
class Params:
    ...
    post_pilot_d: float = 2.6       # M3 self-tapping; use 4.0 for an M3 heat-set insert
    post_pilot_depth: float = 14.0
    ...
    def z_deck(self) -> float:
        """Underside of the deck plate (= top of the posts)."""
...
def make_base(p: Params) -> cq.Workplane:
    ...
    post_h = p.z_deck - p.base_t
    for x, y in corners(p.post_c):
        post = box(p.post_w, p.post_w, post_h + 0.5, x, y, p.base_t - 0.5).edges("|Z").fillet(1.5)
        base = base.union(post)
        base = base.cut(cyl(p.post_pilot_d, p.post_pilot_depth + 1, x, y, p.z_deck - p.post_pilot_depth))
...
def run_checks(p: Params, parts: dict[str, cq.Workplane]) -> dict:
    ...
    results = {
        ...
        "heights_mm": {
            ...
            "deck underside / post top": p.z_deck,
