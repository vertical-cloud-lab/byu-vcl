# byu-vcl
Vertical Cloud Lab @ BYU

- [Thermo-Calc input for the 100 g ultrasonic atomization mixture](thermo-calc-100g-mixture-composition.md)
- [Precedents for enterprise/academic interactions](enterprise-academic-precedents.md)
- [BYU rules on VC money and founder involvement](byu-vc-founder-rules.md)
- [Approval steps and an example COI management plan](byu-coi-management-plan-example.md) — also as a typeset [PDF](byu-coi-management-plan-example.pdf) ([LaTeX source](latex/byu-coi-management-plan-example.tex))
- [BYU faculty-founder precedents and contacts](byu-founder-contacts.md)
- [BYU TTO technology catalog and process documents](byu-tto-technology-catalog.md)
