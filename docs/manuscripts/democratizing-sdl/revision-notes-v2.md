...
7. **Zenodo DOIs** for all ten projects, so the cover letter can state that every contributed project is citable, versioned and archived. This answers objection 3 emphatically rather than minimally. **One of the ten is already done and was simply not recorded:** P10 IvoryOS has a Zenodo software deposit (Zhang & Hein, 2025-04-24), concept DOI [10.5281/zenodo.15272617](https://doi.org/10.5281/zenodo.15272617), now cited in Table 1. Worth asking each team whether a deposit already exists before asking them to mint one — nine to chase, not ten.

8. **Two of the four public design-file repositories carry no licence.** P2 (`owen-melville/photo-reactor`) and P4 (`auto_lab/47332-student-excercises`) have no `LICENSE` file at their root, checked 2026-09-14. Under the OSHWA definition this Perspective cites as ref 9 — and leans on in §6 to define open hardware — published files without a licence grant are not open hardware, and default copyright leaves a replicator without permission to build from them. This is the paper's own thesis applying to the paper's own evidence, so a reviewer will notice; Table 5 currently scores P2 as Procure ● Build ● without reference to it. Two ways to resolve, and the choice belongs to the co-authors, not to this draft: have the teams add a licence (CERN-OHL-S or CC BY-SA for hardware, MIT/Apache-2.0 for the code), which is the better outcome and takes minutes; or add a licence column to Table 5 and report it honestly. Do not do neither. For reference, the projects that do carry one: P5 Apache-2.0/MIT, P6 MIT, P9 MIT, P10 MIT.

9. **P9's repository has been renamed** from `AccelerationConsortium/ac-training-lab` to `ac-dev-lab`. The cited documentation URL <https://ac-training-lab.readthedocs.io/> still resolves HTTP 200, so nothing in Table 1 is broken today, but the underlying repository name in the citation is stale and GitHub's redirect is not a guarantee. Confirm with the AC team which name is permanent before submission.
10. **Author-contribution statement** updated for the §4 analysis and §6 audit, which are new to this version.
11. **Brenden's "manifesto"**, referred to in the issue thread but not findable publicly — needed before it can be folded in.
12. **Confirm ChemRxiv v1 is what was submitted.** *Half-answered.* The as-submitted Word source is now in `submitted/` and `manuscript-v1.md` matches it to 97.8 % of words, with every residual difference a typographic normalisation (see the README). So the ChemRxiv PDF and the arXiv submission are the same text. But that source was created 2025-02-04, nearly a month *after* the 2025-01-09 rejection, so it still does not establish that either matches the December 2024 manuscript the editor read. Only Brenden can settle that, from the submission record.
...
- Link rot re-checked 2026-09-14: all eight project URLs in Table 1 return HTTP 200. The `ac-training-lab` → `ac-dev-lab` repository rename (item 9 above) is the one latent risk.
