...
## Lock behavior (observed 2026-08-12)

While the owner has the deck open, path A returns HTTP 423 continuously (the
co-authoring lock is held the whole session, not just during saves, and
lingers ~10 min after the last editor closes). Safe update pattern for
programmatic edits: loop { fresh download -> re-apply modification -> attempt
PUT }, so a successful upload is always based on the latest server copy and
never clobbers concurrent human edits. See
`scripts/cnms2026_add_timeline.py` for a reusable example.
