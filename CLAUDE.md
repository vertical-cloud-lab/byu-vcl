...
**Slot 7 at read Z 120 is the best measurement pose found so far, and every millimetre up
makes it worse.** Measured 2026-09-09 across five empty-slot scans; the slot 7 rows are a
clean height series in which only `--read-z` changed.

| pose | aperture | mean total | read-to-read spread | spectral disagreement between the 3 X stops |
| --- | --- | --- | --- | --- |
| slot 7, z 120 | 29.5 mm | **7069** | **0.03-0.07%** | **0.8%** |
| slot 7, z 125 | 34.5 mm | 5203 | 1-11% | 15.2% |
| slot 7, z 128 | 37.5 mm | 3147 | 3-11% | 55.7% |
| slot 8, z 120 | 29.5 mm | 4527 | 0.1-28% | -- |

The last column is the one that matters for a colour test: the largest relative difference,
between the three X stops, in any channel's *share* of the total -- i.e. how much the
instrument disagrees with itself about the **colour** of an empty slot once intensity is
divided out. At z 120 it is 0.8%; at z 128 it is 56%, which is bigger than the difference
between two of the paints. **Do not raise the aperture looking for a better view -- there
isn't one.** Lower than 120 may well be better, but 120 is the lowest read height ever run
with the module on the nozzle, so going below it leaves the proven envelope and wants a
human watching the first descent.

Two claims from the 2026-09-09 z 125 session do not survive the third height, and are
corrected here rather than left in the results file:

- *"The spectral shape is identical in all three conditions"* was an eyeball reading of the
  plotted curves. Computed, it holds only at z 120. The gross shape -- a rise to a 583/620 nm
  peak -- persists at every height; the proportions do not.
- *The inverse-square fit* (`(29.5/34.5)^2 = 0.731` predicted vs 0.726 measured at the centre
  position) was one point out of six. Extended to z 128 the measured ratios are 0.504 / 0.344
  / 0.487 against a predicted 0.619. There is no single point source: raising the aperture
  changes *which* ambient light reaches it, not just how much.
