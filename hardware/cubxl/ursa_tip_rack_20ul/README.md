# Cub-XL tip rack — 20 µL variant (Ø3.75 → Ø3.45 taper, Ø2.5 exit bore)
...
| `TipRack20uL.png` | Preview above — print orientation, top-down section, and the measured bore profile against the stock one. |
...
| --- | --- | --- |
| Taper, top → base | Ø6.980 → Ø6.424 | **Ø3.750 → Ø3.450** |
| Taper length | 16 mm | **16 mm** |
| Taper half-angle | 0.99° | **0.54°** |
| Exit bore | Ø4.32 × 2 mm | **Ø2.500 × 2 mm** |
...
- Bore diameters ray-measured off the exported mesh, across flats:
  ...
  | Depth below top face | Nominal | Measured (min) |
  | --- | --- | --- |
  | 0 mm (mouth) | 3.7500 | **3.7500** |
  | 4 mm | 3.6750 | **3.6750** |
  | 8 mm | 3.6000 | **3.6000** |
  | 12 mm | 3.5250 | **3.5250** |
  | 16 mm (base of taper) | 3.4500 | **3.4502** |
  | 16–18 mm (exit bore) | 2.5000 | **2.5000** |

  All 30 bores measure 3.7500 at the mouth and 2.5000 at the exit. The facet
...
- Volume 85 076.4 mm³ (85.08 cm³), within 1.9 mm³ of the analytic prediction (facet discretisation).
...
down to the Ø2.5 exit bore faces upward, so it needs no support either; the only
...
- **Check the fit before printing all 30.** Most FDM printers come out 0.1–0.2 mm
  undersize on vertical holes, and at Ø3.75 that is a 3–5 % error — enough to
  matter. If your 20 µL tips bind, regenerate with the taper shifted up by the
  measured error, keeping the 0.30 mm difference between the ends:

  ```bash
  pip install trimesh manifold3d numpy lxml networkx
  python make_tip_rack_20ul.py --top-diameter 3.95 --bottom-diameter 3.65
  ```
...
matches the new, much smaller taper — a different height than a 300 µL tip sits
at in the stock rack. `pickup_z` and `drop_z` are therefore carried over from
