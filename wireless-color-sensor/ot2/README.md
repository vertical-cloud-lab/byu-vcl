...
- **`--rgb` defaults to `0,0,0`, so every scan so far measured ambient light
  only.** The module's own illumination has never been switched on. For a real
  colour measurement — the diluted-paint test this was built for — set `--rgb`
  and re-baseline. This also reframes the "46% swing" finding: it is a symptom
  of reading with the lights off, not an inherent property of the deck.
...
### 2026-09-09 — slot 7 at z 128: the height series closes, and the vials arrive

A third cycle at the height asked for on #197 (`--scan-slot 7 --read-z 128`),
3 mm above the previous run. Slot 7 empty again, LEDs off again. Cycle complete,
no aborts: grip check 5.3×, reseat confirmation 439 against a seated 440.

Full write-up and data: [`results-height-series-2026-09-09.md`](results-height-series-2026-09-09.md).

With three heights in the same slot, the trend is unambiguous and monotone:
...
| Spectral disagreement between the 3 X stops | **0.8 %** | 15.2 % | 55.7 % |

The last row is the decisive one for a colour test: it is how much the sensor
disagrees with *itself* about the colour of an empty slot at three positions,
with intensity divided out. `--read-z 120` is the default for a reason; do not
raise it. Lower is the untested direction, and 120 is the lowest read height ever
run with the module aboard, so the first descent below it wants a human watching.

Two claims from the z 125 write-up are corrected in `CLAUDE.md`: the spectral
shape is *not* identical at every height (it holds only at z 120), and the
inverse-square fit at the centre position was one point in six — extended to a
third height it fails at every position.

**The three paint vials appeared on the deck during this run** — absent at
01:41 UTC, present at 01:45 (`deck-before-z128-2026-09-09.jpg` vs
`deck-after-z128-2026-09-09.jpg`). They are standing beside the enclosure's base,
not in the scan slot, so this run did not see them.
