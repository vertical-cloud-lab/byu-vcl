...
def _as_datetime(iso):
    """Parse the ISO stamps sensor_read emits; None if a reading predates them."""
    if not iso:
        return None
...
def store_in_mongodb(records, run_meta, uri, database):
    ...
    try:
        ...
        docs = []
        for r in records:
            # timestamp is when the *reading* happened, not when the batch was
            # written. Every document of a run used to share one utcnow() taken
            # after the last read, putting readings up to three minutes late --
            # which is useless for lining a measurement up against anything
            # else on a clock, the livestream included. stored_at keeps the
            # write time, which is the only thing it was ever good for.
            docs.append({
                "timestamp": _as_datetime(r.get("t_response_utc")) or stored_at,
                "t_request": _as_datetime(r.get("t_request_utc")),
                "stored_at": stored_at,
...
def parse_args(argv=None):
    ...
    p.add_argument("--press-z", type=float, default=PRESS_Z,
                   help="nozzle Z at the pickup press; lower = deeper into the "
...
def build_plan(args):
    ...
    if args.read_z <= args.press_z:
        raise SystemExit(
            f"--read-z {args.read_z} is at or below the press depth "
            f"{args.press_z}; the module's foot would be driven into the deck"
        )
    if not PRESS_Z - 2.0 <= args.press_z <= ENTRY_Z:
        raise SystemExit(
            f"--press-z {args.press_z} is outside the range proven safe for the "
            f"socket ({PRESS_Z - 2.0} to {ENTRY_Z}); a deeper press drives the "
            "nozzle through the module and into its base"
        )
    return {
        ...
        # rides that much higher on it. Shifting the release height by the same
        # delta puts the module back down at the absolute height the proven
        # recipe released it from, instead of dropping it from 0.5 mm up.
        "release_z": round(DROP_DESCENT[-1] + (args.press_z - PRESS_Z), 2),
        "aperture_height_mm": round(args.read_z - args.press_z, 2),
    }
...
def pick_up(robot, plan):
    ...
    log(f"press to z={plan['press_z']:g}")
    robot.move(x, y, plan["press_z"], speed=PRESS_SPEED)
...
def reseat(robot, plan, carry_z=CARRY_Z):
    ...
    log(f"staged descent, releasing at z={plan['release_z']:g}")
    ladder = list(DROP_DESCENT[:-1]) + [plan["release_z"]]
    for z in [z for z in ladder if z < carry_z]:
    ...
    try:
        ...
        if args.dry_run:
            ...
            try:
                ...
                if args.align:
                    x, y = plan["pickup"]["x"], plan["pickup"]["y"]
                    robot.move(x, y, plan["press_z"] + 30.0, speed=40.0)
