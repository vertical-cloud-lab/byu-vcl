...
| `stl/fake_tip_enclosure_bore.stl` | **Swap-in tip with the real enclosure's pipette interface** (requested by @timothy-commins in PR #60). Same base as the array tips — body Ø8 × 10 + flange Ø10 × 2, 20 mm total — so it drops into the deck-plate pockets and the lightweight housing exactly like the test-array tips. The post above the flange is an **8 mm exact replica of the reference enclosure post's top section** (`reference/sensor_package_main_enclosure_7p5mm.step`): tapered bore **entry ID 7.50 mm narrowing at 1.589° half-angle** inside a constant 1.5 mm wall (post OD 10.06 → 10.50 mm, widening toward the mouth like the original). The exact bore taper continues 10 mm below the mouth (through the flange), then necks to a Ø4.6 blind nozzle-clearance hole (floor 16 mm below the mouth) so the P20 nozzle can't bottom out before the ejector sleeve wedges. `7.5` is engraved on the underside. Verified by `verify_fake_tip_enclosure_bore()`: section radii match the reference post to **0.001 mm** over the full replica height. |
...
| Enclosure-bore swap-in tip | ![encl bore iso](renders/fake_tip_enclosure_bore_iso.png) | ![encl bore top](renders/fake_tip_enclosure_bore_top.png) |
